...
for index,key in enumerate(csv_graph_dict.keys()):  
    ...
    axs[index] = df.plot(label=key,x="Angle",y="Intensity")
    df.plot.scatter(ax=axs[index],label="Peaks",x="Angle",y="Peaks",c="orange")
    
    if key is "Grinded Morton Salt" or key is "Morton Salt" or key is "Powder Salt and Al":
        for item in dfb["angle"]:
            axs[index].axvline(item,linestyle="--",c="green")
        for index,row in dfb.iterrows():
            pass
            # axs[index].annotate("{}".format(row["hkl"]),(row["angle"] + x_offset,2000),rotation=45)
    
    if key is "Aluminum Foil" or key is "Powder Salt and Al":
        for item in dfa["angle"]:
            axs[index].axvline(item,linestyle="--",c="red")
        for index,row in dfa.iterrows():
            print(row["hkl"])
            # axs[index].annotate("{}".format(row["hkl"]),(row["angle"],2000),rotation=45)
    ...
    for index, row in new_df.iterrows():
        try:
            ...
            pass
        last_row = row["Angle"]
    axs[index].set_xticks(x_tick_list)
    axs[index].set_xticklabels(x_tick_list,rotation=45)
    axs.show()
    fig = plt.gcf()
